...
import dynamic.model.web.spring.boot.annotation.ModelRegister
...
class  ModelPathScan constructor(private val annotation: KClass<out Annotation>, registry: BeanDefinitionRegistry): ClassPathBeanDefinitionScanner(registry, false) {
    ...
    private  fun getModelNameByModelCls(cls: Class<*>): String{
       var ano = AnnotationUtils.findAnnotation(cls, Model::class.java)
        if (ano!=null){
            return ano.name
        }
        return ""
        ...
        packageNames.forEach {
            try {
                ...
                while (iter.hasNext()){
                    ...
                    var ano=AnnotationUtils.findAnnotation(definition.beanClass, Model::class.java)
