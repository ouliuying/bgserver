...
import work.bg.server.core.mq.aggregation.AggExpression
...
import work.bg.server.core.mq.condition.constant.BooleanExpression
import work.bg.server.core.mq.condition.constant.StringExpression
...
class ModelCriteriaRender:ModelExpressionVisitor{
    ...
    private fun buildIsExpression(expression: ModelExpression?,parent: ModelExpression?){
        if (expression is IsExpression){
            ...
            this.namedParameters[columnName]=FieldValue((expression as IsExpression).field,"%"+(expression as IsExpression).value+"%")
        }
        else{
            ...
            var namedColumnName=this.tableColumnNameGenerator.generateNamedParameter(columnName)
            this.namedSql.append(namedColumnName)
            this.namedParameters[columnName]=FieldValue((expression as IsNotExpression).field,"%"+(expression as IsNotExpression).value+"%")
        }
    }
    private  fun buildLikeExpression(expression: ModelExpression?,parent: ModelExpression?){
        if (expression is LikeExpression){
            ...
            var namedColumnName=this.tableColumnNameGenerator.generateNamedParameter(columnName)
            this.namedSql.append(namedColumnName)
            this.namedParameters[columnName]=FieldValue((expression as LikeExpression).field,"%"+(expression as LikeExpression).value+"%")
        }
        else{
            ...
            var namedColumnName=this.tableColumnNameGenerator.generateNamedParameter(columnName)
            this.namedSql.append(namedColumnName)
            this.namedParameters[columnName]=FieldValue((expression as NotLikeExpression).field,"%"+(expression as NotLikeExpression).value+"%")
        }
    }
    private  fun buildInExpression(expression: ModelExpression?,parent: ModelExpression?){
        if (expression is InExpression){
            ...
            this.namedSql.append(GRAMMARKEYS.BRACKET_PREFIX)
            if((expression as InExpression).valueSet!=null)
            {
                var namedColumnName=this.tableColumnNameGenerator.generateNamedParameter(columnName)
                this.namedSql.append(namedColumnName)
                this.namedParameters[columnName]=FieldValue((expression as InExpression).field,(expression as InExpression).valueSet)
            }
            else{
            ...
            if((expression as NotInExpression).valueSet!=null)
            {
                var namedColumnName=this.tableColumnNameGenerator.generateNamedParameter(columnName)
                this.namedSql.append(namedColumnName)
                this.namedParameters[columnName]=FieldValue((expression as NotInExpression).field,(expression as NotInExpression).valueSet)
            }
            else{
    ...
    private fun buildCheckValueExpression(checkValueExpression: CheckValueExpression,parent:ModelExpression?){
        ...
        when(checkValueExpression.value){
            is FieldBase->{
                ...
                this.namedParameters.put(columnName, FieldValue(checkValueExpression.field,checkValueExpression.value))
            }
        }
    }
